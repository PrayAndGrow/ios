...
- (void)layoutSubviews {
    if (shouldUpdateSubviews == YES) {
        ...
        self.timeChanged(@(self.hours),@(self.minutes), @(self.seconds));
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.enableDigit == YES) {
        ...
        for(unsigned i = 0; i < 60; i+=5){
            NSString *minuteNumber = [NSString stringWithFormat:@"%@%d", [NSString stringWithFormat:@"%@", i+1<10 ? @" ": @""] , i];
            CGFloat labelX = center.x + (markingDistanceFromCenter - self.digitFont.lineHeight/2.0f) * cos((M_PI/180)* (i/5+offset) * 30 + M_PI);
            CGFloat labelY = center.y + - 1 * (markingDistanceFromCenter - self.digitFont.lineHeight/2.0f) * sin((M_PI/180)*(i/5+offset) * 30);
            [minuteNumber drawInRect:CGRectMake(labelX - self.digitFont.lineHeight/2.0f,labelY - self.digitFont.lineHeight/2.0f,self.digitFont.lineHeight,self.digitFont.lineHeight) withAttributes:@{NSForegroundColorAttributeName: self.digitColor, NSFontAttributeName: self.digitFont}];
        }
    }
}
@end
